...
def rho_13(del_om, del_om_p, rabi_p, rabi_c):
    """
    ...
    :return: 
    """
    coef = 2 * param.ionic_density * pc.mu0 * (0.063 * pc.muB)**2 / (pc.hbar * rabi_p)
    return coef * 2 * del_om * rabi_p / (4 * del_om * del_om_p - rabi_c**2 + 2 * 1j * del_om * param.gamma)
...
detunings = sp.linspace(-10e6, 10e6, 3000)
response_bare = rho_13(detunings + offset_c, detunings, rabi_p, rabi_c)
response = sp.zeros(3000, dtype=complex)
single_reponse = rho_13(detunings + offset_c, detunings, rabi_p, rabi_c)

for i in inhomogeneous:
        response = response + rho_13(detunings + i, detunings + offset_c, rabi_p, rabi_c)
